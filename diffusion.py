...
class UNET(nn.Module):
    ...
    def forward(self, x, context, controlnet_outputs, time):
        ...
        x += controlnet_outputs.pop()
        
        for layers in self.decoders:
            # Since we always concat with the skip connection of the encoder, the number of features increases before being sent to the decoder's layer
            x = torch.cat((x, skip_connections.pop() + controlnet_outputs.pop()), dim=1) 
            x = layers(x, context, time)
...
class Diffusion(nn.Module):
    ...
    def copy_diffusion_weights_to_controlnet(self):
        
        self.controlnet.time_embedding.load_state_dict(self.time_embedding.state_dict())
        self.controlnet.unet.load_state_dict(self.unet.state_dict(), strict=False)
    
    def forward(self, latent, cloth_agnostic_mask,
                densepose, cloth, resized_agn_mask, canny, image_embeddings, time, is_train=True, do_cfg=True):
        ...
        # (batch_size, 20, Height / 8, Width / 8)
        controlnet_inputs = canny
        
        # For classifier-free guidance in Inference
        if is_train == False and do_cfg == True:
            latent = latent.repeat(2, 1, 1, 1)
            controlnet_inputs = controlnet_inputs.repeat(2, 1, 1, 1)
        ...
        if is_train and do_cfg:
            ...
            if probabilities < self.p_uncond:
                controlnet_outputs = self.controlnet(latent, controlnet_inputs, 
                                                     self.uncond_vector.to(image_embeddings.device).repeat(image_embeddings.shape[0], 1, 1), 
                                                     time)
                output = self.unet(latent, self.uncond_vector.to(image_embeddings.device).repeat(image_embeddings.shape[0], 1, 1), 
                                   controlnet_outputs, time)
            else: 
                controlnet_outputs = self.controlnet(latent, controlnet_inputs, 
                                                     image_embeddings,
                                                     time)
                output = self.unet(latent, image_embeddings, controlnet_outputs, time)
        # (is_train and do_cfg == False) or is_train == False 
        else: 
            controlnet_outputs = self.controlnet(latent, controlnet_inputs, image_embeddings, time)
            output = self.unet(latent, image_embeddings, controlnet_outputs, time)
